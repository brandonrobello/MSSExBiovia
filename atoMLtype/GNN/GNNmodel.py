...
class BaselineGCN(nn.Module):
    """
    A baseline Graph Convolutional Network (GCN) for atom type classification.

    This model uses two GCN layers followed by a fully connected layer for classification.
    """

    def __init__(self, num_node_features, num_atom_types, hidden_dim=64):
        """
        Initializes the BaselineGCN model.
    ...
    def forward(self, data, return_graph_embedding=False):
        """
        Forward pass of the BaselineGCN.
...
class BaselineGAT(nn.Module):
    """
    A baseline Graph Attention Network (GAT) for atom type classification.

    This model uses two GAT layers followed by a fully connected layer for classification.
    """
    ...
    def __init__(self, num_node_features, num_atom_types, hidden_dim=64, heads=4):
        ...
        Args:
            ...
            heads (int): Number of attention heads in the GAT layers.
        """
        super().__init__()
    ...
    def forward(self, data, return_embeddings=False):
        """
        Forward pass of the BaselineGAT.

        Args:
            data (Data): PyTorch Geometric Data object containing graph data.
            return_embeddings (bool): If True, returns node embeddings.

        Returns:
            Tensor: Predicted atom types or node embeddings.
        """
        x, edge_index = data.x, data.edge_index
...
class BaselineGIN(nn.Module):
    """
    A baseline Graph Isomorphism Network (GIN) for atom type classification.

    This model uses two GIN layers with MLPs followed by a fully connected layer for classification.
    """
    ...
    def __init__(self, num_node_features, num_atom_types, hidden_dim=512, dropout=0.2):
        ...
        Args:
            num_node_features (int): Number of input node features.
            num_atom_types (int): Number of output atom type classes.
            hidden_dim (int): Hidden dimension for the GIN layers.
    ...
    def forward(self, data):
        """
        Forward pass of the BaselineGIN.

        Args:
            data (Data): PyTorch Geometric Data object containing graph data.

        Returns:
            Tensor: Predicted atom types.
        """
        x, edge_index = data.x, data.edge_index
        x = F.elu(self.nn1(x, edge_index))
        x = F.elu(self.nn2(x, edge_index))
